...
class baseline_dataloader_train_strong(Dataset):
    ...
    def __init__(self, params, shuffle = True, data_percentage = 1.0, split = 1, frame_wise_aug = False, no_aug= False, dl_mode = '20p', label_mode = 'gt'):
        ...
        if self.dataset == 'ucf101':
            ...
            self.all_paths = open('data/k400train_full_path_resized_annos.txt','r').read().splitlines()
        else:
    ...
    def process_data(self, idx):
        ...
        # label_building
        if self.dataset == 'ucf101':
            vid_path = cfg.path_folder + '/UCF-101/' + self.data[idx].split(' ')[0][41:]
            if self.label_mode == 'gt':
                label = self.classes[vid_path.split('/')[-2]] # This element should be activity name
            else:
                label = np.mean(self.soft_labels[os.path.basename(vid_path)], axis =0) 
...
class multi_baseline_dataloader_val_strong(Dataset):
    ...
    def __init__(self, params, shuffle = True, data_percentage = 1.0, mode = 0, \
        ...
        if self.dataset=='ucf101':
            ...
            if retrieval_train:
                ...
                # print('here')
                self.all_paths = open('data/k400val_full_path_resized_annos.txt','r').read().splitlines()
                
                
        elif self.dataset=='casia':
            
            if self.casia_split == 'g':
                self.all_paths = open('data/casia-b/gallery_vids.txt','r').read().splitlines()
            elif self.casia_split == 'nm':
                self.all_paths = open('data/casia-b/prob_vids_nm.txt','r').read().splitlines()
            elif self.casia_split == 'cl':
                self.all_paths = open('data/casia-b/prob_vids_cl.txt','r').read().splitlines()
            elif self.casia_split == 'bg':
                self.all_paths = open('data/casia-b/prob_vids_bg.txt','r').read().splitlines()
    ...
    def process_data(self, idx):
        ...
        if self.dataset == 'ucf101':
            ...
            vid_path1 = cfg.path_folder.replace('/paths_crcv_node', '')  + '/HMDB51/' + vid_path.split(' ')[1]+ '/' + vid_path.split(' ')[0]
